...
class AdmittanceConfig:
    ...
    admittance_damping_z: float = 60.0  # symmetric D [N·s/m] (fallback)
    # Asymmetric D (Peirastic / soft-tissue): heavier when pressing into contact
    # (v_force_z ≤ 0), lighter when retracting (v_force_z > 0).  Omit press/release
    # to keep symmetric admittance_damping_z only.
    admittance_damping_z_press: float | None = None   # into tissue [N·s/m]
    admittance_damping_z_release: float | None = None  # lift off [N·s/m]
    # Peirastic force-derivative damping: M·v̇ + D·v + Kdf·Ḟ_ext = F_err  [Kdf: s]
    admittance_force_derivative_gain_z: float = 0.0
    ...
    # Yanan Li / pHRI: online K_e → b_d = 2ζ√(m_d K̂_e) for fixed ζ (critical when ζ=1).
    ...
    def from_dict(cls, raw: dict) -> AdmittanceConfig:
        ...
        return cls(
            ...
            admittance_damping_z_press=(
                float(c["admittance_damping_z_press"])
                if c.get("admittance_damping_z_press") is not None
                else None
            ),
            admittance_damping_z_release=(
                float(c["admittance_damping_z_release"])
                if c.get("admittance_damping_z_release") is not None
                else None
            ),
            admittance_force_derivative_gain_z=float(
                c.get(
                    "admittance_force_derivative_gain_z",
                    c.get("force_derivative_gain_z", 0.0),
                )
            ),
...
class AdmittanceController:
    ...
    def __init__(self, dt: float, config: AdmittanceConfig | None = None) -> None:
        ...
        self._f_ext_z_prev = 0.0
        self._f_ext_z_valid = False
    ...
    def compute_velocity_command(
        ...
        *,
        ...
        self._f_ext_z_prev = f_ext_z
        self._f_ext_z_valid = True
        ...
        for axis in range(6):
            ...
            v_force_tool[axis] = self._admittance_axis(
                ...
                f_dot_z=f_dot_z if axis == 2 else 0.0,
            )
    ...
    def _directional_damping_z(self) -> float:
        """
        Peirastic-style asymmetric D on tool-Z (test_spacenav_admittance.py):

            Dd_eff = D_up if adm_vel > 0 else D_down   (D_down > D_up)

        Heavier damping while pressing into soft tissue suppresses penetration
        overshoot; lighter damping on retract avoids snap-back limit cycles.
        """
        cfg = self.cfg
        d_sym = float(cfg.admittance_damping_z)
        d_press = float(cfg.admittance_damping_z_press if cfg.admittance_damping_z_press is not None else d_sym)
        d_release = float(cfg.admittance_damping_z_release if cfg.admittance_damping_z_release is not None else d_sym)
        if d_press == d_release:
            return d_sym
        ...
        *,
        ...
        if cfg.adaptive_ke.enabled:
            ...
            if cfg.var_damping_enabled:
                d = d_base + cfg.var_damping_d_u * self.instability_index
            else:
                d = d_base
    ...
    def _admittance_axis(
        ...
        *,
        ...
        cfg = self.cfg
        if axis == 2:
            return self._admittance_z(f_err, in_contact, db_alpha, f_dot_z=f_dot_z)
