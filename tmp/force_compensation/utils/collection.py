...
def run_cartesian(bot, cfg: ForceIdConfig, slot: str) -> Path:
    ...
    exc = ex.CartesianExcitation.from_config(cart, c.scale)
...
def run_pose_d(bot, cfg: ForceIdConfig) -> Path:
    ...
    try:
        for i in range(n_total):
            ...
            if t_cmd < pd.joint_duration_s:
                ...
                ramp_b = min(1.0, t_burst / pd.burst.ramp_s) if pd.burst.ramp_s > 0 else 1.0
                delta = ex.inertia_burst_delta(t_burst, pd.burst) * ramp_b
                send_pose_canfd(
                    bot.robot, (pose0 + delta).tolist(),
                    follow=c.follow, trajectory_mode=0, radio=0,
                )
    ...
    np.savez(
        ...
        pose0=pose0, q0_deg=q0, pose_slot="d",
        preset="joint+inertia_burst", scale=c.scale,
        joint_s=pd.joint_duration_s, burst_s=pd.burst_duration_s,
        dt_ms=c.dt_ms, log_every=c.log_every, method="joint+inertia_burst",
    )
    return out


def slot_kind(slot: str) -> str:
    return "joint+burst" if slot == "d" else "cartesian"
...
def dry_run(cfg: ForceIdConfig) -> None:
    ...
    for slot in seq:
        ...
        print(f"  {slot} [{slot_kind(slot)}]: {rec.get('label', f'pose_{slot}')}")
...
def main(argv: list[str] | None = None) -> int:
    ...
    for s in seq:
        print(f"  {s} [{slot_kind(s)}]: {slots[s][2].get('label', f'pose_{s}')}")
