...
def sin_y_motion(
    t_s: float, amplitude_m: float, omega: float, *, soft_start: bool
) -> tuple[float, float]:
    if soft_start:
...
class LagStats:
    ...
    def update(self, y_ref: float, y_fb: float, x_fb: float, z_fb: float, x0: float, z0: float) -> None:
        self.n += 1
    ...
    def line(self, t: float, vy_cm_s: float) -> str:
        if self.n == 0:
            return ""
        return (
            f"  t={t:.1f}s vy={vy_cm_s:+.2f}cm/s  "
            f"Y err max={self.max_y_err_mm:.1f}mm  drift tool X={self.max_x_drift_mm:.1f} Z={self.max_z_drift_mm:.1f}mm"
        )
...
def main() -> int:
    ...
    soft_start = bool(traj_cfg.get("soft_start", True))
    amp_m = amp_mm / 1000.0
    ...
    print(
        ...
        f"period={period:.1f}s  soft_start={soft_start}  hold={hold_s:.1f}s",
        flush=True,
    )
        ...
        try:
            while True:
                ...
                if t_now < hold_s:
                    ...
                    dy_cmd, vy_cmd = sin_y_motion(t_scan, amp_m, omega, soft_start=soft_start)
                    y_ref_cmd = y0 + dy_cmd
                    t_fb = last_fb_t - t_start - hold_s
                    dy_fb, _ = sin_y_motion(max(0.0, t_fb), amp_m, omega, soft_start=soft_start)
                    y_ref_fb = y0 + dy_fb
                ...
                if t_now >= hold_s and now - last_log >= 1.0:
                    last_log = now
                    stats.update(y_ref_cmd, last_fb[1], last_fb[0], last_fb[2], x0, z0)
                    print(stats.line(t_now, vel[Y_AXIS] * 100.0), flush=True)
                    stats = LagStats()
